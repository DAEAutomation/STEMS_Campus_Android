...
@Composable
fun refundHistoryScreen(navController: NavHostController, histories: List<HistoryModel.RefundHistory>, historyViewModel: HistoryViewModel = hiltViewModel(), onShowTabBarChange: (Boolean) -> Unit) {
    ...
    refundHistoryContent(
        ...
        rowClickHandled = { detail ->
            historyViewModel.refundHistoryDetail = detail
            navController.navigate("RefundDetailHistory")
        }
    )
}
...
@Composable
private fun refundHistoryContent(
    navController: NavHostController,
    onShowTabBarChange: (Boolean) -> Unit,
    histories: List<HistoryModel.RefundHistory> = emptyList(),
    rowClickHandled:(HistoryModel.RefundHistory) -> Unit) {
                    ...
                    Column (modifier = Modifier.verticalScroll(rememberScrollState())){
                        ...
                        grouped.forEach { (ym, items) ->
                            ...
                            items.forEachIndexed { index, item ->
                                ...
                                refundHistoryRow(item = item, shape = shape, rowClick = { detail -> rowClickHandled(detail)})
...
@Composable
private fun refundHistoryRow(
    item: HistoryModel.RefundHistory,
                ...
                Surface(
                    ...
                    Row {
                        Column {
                            ...
                            Row {
                                Text("${item.amount?.toAmountString()}", color = Color.Black, style = MaterialTheme.typography.titleMedium)
                                Text("${stringResource(R.string.currency_unit)}", color = Color.Black, style = MaterialTheme.typography.titleMedium)
                            }
...
@Composable
private fun refundHistoryPreview() {

// 創建一個模擬的 NavController
    val navController = TestNavHostController(LocalContext.current)
    refundHistoryContent (navController,{},emptyList(),{})
}